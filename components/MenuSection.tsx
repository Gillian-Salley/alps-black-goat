...
export default function MenuSection() {
  ...
  return <section className="menu section" id="menu">
    ...
    <div className="grid">{visibleItems.map((item) => <article key={item.name}><div><h3>{item.name} <small>{item.unit}</small>{item.badge && <i>{item.badge}</i>}</h3><p>{item.description}</p></div><strong>{item.price}</strong></article>)}</div>
